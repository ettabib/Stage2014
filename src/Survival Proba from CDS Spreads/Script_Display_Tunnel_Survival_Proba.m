...
%%% DISPLAY UNION RECTANGLES %%%%%

figure;
hold on;
Gray_map = colormap('Gray');
Color = Gray_map(end-10, :)

X_inf = 0;
Y_inf = Bounds_Large(1,1);
T_1 = t_pillars(1);
Width = T_1-0;
Height = 1 - Bounds_Large(1,1);
    
rectangle('Position',[X_inf,Y_inf,Width,Height],...
          'Curvature',[0,0],...
         'LineWidth',1,'LineStyle','-','FaceColor',Color)
...
for i = 2:Nb_pillars
    ...
    T_i = t_pillars(i);
    T_i_minus_1 = t_pillars(i-1);
    %line([T_i+eps T_i+eps], [Bounds_Large(i,1) Bounds_Large(i,2)],'LineWidth',3, 'Color', 'r')
    %line([T_i T_i], [Bounds_Large(i,1) Bounds_Large(i,2)],'LineWidth', 3, 'Color', 'k')    
    
    
    X_inf = T_i_minus_1
    Y_inf = Bounds_Large(i,1);
    Width = T_i-T_i_minus_1;
    Height = Bounds_Large(i-1,2) - Bounds_Large(i,1);
...
%line([T_1 T_1], [Bounds_Large(i,1) Bounds_Large(i,2)],'LineWidth', 2, 'Color', 'k')
for i = 1:Nb_pillars
