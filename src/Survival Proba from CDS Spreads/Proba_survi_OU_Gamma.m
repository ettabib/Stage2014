function res  = Proba_survi_OU_Gamma(t_scheduled,  intensities, t_pillars, Gamma_OU_Param)
% --------------------------------------------------------------------------------------------------
% Compute survival probability in a reduced-from model where intensities
% driven by a Gamma-OU process with piecewise constant mean-reversion
% coefficient
% --------------------------------------------------------------------------------------------------
...
% t_pillars = [2;3;5]
% res  = Proba_survi_OU_Gamma(t_scheduled,intensities, t_pillars, Gamma_OU_Param)
% -------------------------------------------------------------------------
% -------------------------

x = Gamma_OU_Param(1);
a = Gamma_OU_Param(2);
sigma = Gamma_OU_Param(3);
c = Gamma_OU_Param(4);
lambda = Gamma_OU_Param(5);
...
for k = 1:Nb_period
    ...
    Coeff_c = zeta_Gamma(t, 0, a, sigma, lambda);
    %Coeff_c = zeta_Gamma_exact(t, 0, a, sigma, lambda);
